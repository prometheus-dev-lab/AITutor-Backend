...
class ChatHistoryTests(unittest.TestCase):
    def test_chat_history_initialization(self):
        ...
        self.assertEqual(
            ...
            1,
            "Chat history should initially contain one entry.",
        )
    ...
    def test_adding_user_response(self):
        ...
        self.assertEqual(
            len(chat_history.chat),
            2,
            "Chat history should contain two entries after adding user response.",
        )
    ...
    def test_adding_tutor_response(self):
        ...
        self.assertEqual(
            len(chat_history.chat),
            2,
            "Chat history should contain two entries after adding tutor response.",
        )

    def test_env_string_output(self):
        chat_history = ChatHistory()
        self.assertIsInstance(
            chat_history.env_string(), str, "env_string should return a string."
        )

    def test_serialization_to_sql(self):
        chat_history = ChatHistory()
        self.assertIsInstance(
            chat_history.to_sql(), str, "to_sql should return a string."
        )
    ...
    def test_deserialization_from_sql(self):
        ...
        self.assertEqual(
            original_chat_history.chat,
            new_chat_history.chat,
            "Deserialized chat history should match original.",
        )

    def test_json_format(self):
        chat_history = ChatHistory()
        self.assertIsInstance(
            chat_history.format_json(), dict, "format_json should return a dictionary."
        )
