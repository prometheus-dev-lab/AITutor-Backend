...
class PromptTests(unittest.TestCase):
    def test_prompt_parsing_from_llm_output(self):
        ...
        self.assertEqual(
            p1.format_json()["type"],
            PromptAction.Type.TEXT.value,
            "Error in parsing Text prompt.",
        )

        p2 = PromptAction.parse_llm_action(json_output2)
        self.assertEqual(
            p2.format_json()["type"],
            PromptAction.Type.TERMINATE.value,
            "Error in parsing Termination prompt.",
        )

        p3 = PromptAction.parse_llm_action(json_output3)
        self.assertEqual(
            p3.format_json()["type"],
            PromptAction.Type.FILE.value,
            "Error in parsing File prompt.",
        )

        p4 = PromptAction.parse_llm_action(json_output4)
        self.assertEqual(
            p4.format_json()["type"],
            PromptAction.Type.RATING.value,
            "Error in parsing Rating prompt.",
        )
