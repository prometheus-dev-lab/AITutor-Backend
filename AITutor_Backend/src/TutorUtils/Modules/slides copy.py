...
class Slide(JSONSerializable, SQLSerializable):
    ...
    def __init__(
        ...
        dialogue: str,
        content: str,
        latex_codes: List[Tuple[str, str]],
        purpose: Purpose,
    ...
    def parse_llm_for_content(llm_output: str):
        ...
        try:
            ...
            return (
                False,
                f"Error parsing JSON on output: {llm_output},  Ensure your response was in JSON Format",
            )
        s_content = slideplan_data.get("content", "content was not a valid entry.")
        if s_content and isinstance(s_content, list):
            s_content = "\n".join(s_content)
        return "content" in slideplan_data, s_content

    @staticmethod
    def parse_llm_for_dialogue(llm_output: str):
        """
        ...
        return "presentation" in slideplan_data, slideplan_data.get(
            "presentation", "presentation was not a valid entry"
        )
...
class SlidePlan(JSONSerializable):
    ...
    def create_slideplan_from_JSON(llm_output, ConceptDatabase):
        ...
        assert (
            title and type(title) == str
        ), "Invalid or missing title in SlidePlan JSON data"

        # Validating the purpose
        purpose = int(slideplan_data.get("purpose", -1))
        assert (
            purpose != -1 and 0 <= purpose <= 4
        ), "Purpose did not map correctly, ensure you have provided a valid ENUM Value."

        # Validating the purpose statement
        purpose_statement = slideplan_data.get("purpose_statement")
        assert (
            purpose_statement and type(purpose_statement) == str
        ), "Invalid or missing purpose statement in SlidePlan JSON data"

        # Validating the concepts
        concept_names = slideplan_data.get("concepts")
        assert (
            concept_names and type(concept_names) == list
        ), "Invalid or missing concepts in SlidePlan JSON data"
        ...
        assert (
            concepts
        ), "One or more concepts are not recognized in SlidePlan JSON data"
...
class SlidePlanner(JSONSerializable, SQLSerializable):
    class SlideLLMAPI:
        ...
        EXPLORED_CONCEPTS_DELIMITER = (
            "$ENV.CONCEPTS_EXPLORED_VALUES$"  # Environment for the chat history
        )
        ...
        def __init__(
            self,
            slideplan_plan_prompt_file,
            slideplan_plan_to_obj_prompt_file,
            sideplan_terminate_prompt_file,
            slide_description_prompt_file,
        ):
            ...
            self._slideplan_plan_prompt = PromptTemplate.from_file(
                slideplan_plan_prompt_file,
                {
                    "slide_plan_set": SlidePlanner.SlideLLMAPI.LESSON_PLAN_DELIMITER,
                    "explored_concepts": SlidePlanner.SlideLLMAPI.EXPLORED_CONCEPTS_DELIMITER,
                    "notebank_state": SlidePlanner.SlideLLMAPI.NOTEBANK_STATE_DELIMITER,
                },
            )
            """vars: slide_plan_set, explored_concepts, notebank_state"""

            self._slideplan_plan_to_obj_prompt = PromptTemplate.from_file()

        def request_output_from_llm(
            self, prompt, model: str, max_length=2500, temp=0.5
        ):
        ...
        def conversational_JSON_request(
            self, system, assistant, user, model: str, max_length=4000, temp=0.5
        ):
            """Requests the Concept information from an LLM.
        ...
        def prompt_plan_slideplan(
            self, side_plan_set, explored_concepts, notebank_state
        ):
            env_state = {
                SlidePlanner.SlideLLMAPI.LESSON_PLAN_DELIMITER: side_plan_set,
                SlidePlanner.SlideLLMAPI.EXPLORED_CONCEPTS_DELIMITER: explored_concepts,
                SlidePlanner.SlideLLMAPI.NOTEBANK_STATE_DELIMITER: notebank_state,
            }
            return self._load_prompt(self.__slideplan_plan_prompt, env_state)

        def prompt_terminate_slideplan(
            self, side_plan_set, explored_concepts, notebank_state
        ):
            env_state = {
                SlidePlanner.SlideLLMAPI.LESSON_PLAN_DELIMITER: side_plan_set,
        ...
        def prompt_sideplan_description(
            self, slide_plan_ref: SlidePlan, notebank_str: str
        ):
            ...
            env_state = {
                SlidePlanner.SlideLLMAPI.CURR_ENV_SLIDE_PLAN: slide_plan_ref.env_string(),
                SlidePlanner.SlideLLMAPI.CURR_ENV_CONEPT_DATA_DELIMITER: get_concept_data_str(
                    slide_plan_ref
                ),
                SlidePlanner.SlideLLMAPI.NOTEBANK_STATE_DELIMITER: notebank_str,
            }
    ...
    def __init__(self, Notebank, ConceptDatabase):
        ...
        self.llm_api = SlidePlanner.SlideLLMAPI(
            "AITutor_Backend/src/TutorUtils/Prompts/KnowledgePhase/Slides/slideplan_plan_prompt",
            "AITutor_Backend/src/TutorUtils/Prompts/KnowledgePhase/Slides/slideplan_to_obj_prompt",
            "AITutor_Backend/src/TutorUtils/Prompts/KnowledgePhase/Slides/slide_plan_termination_prompt",
            "AITutor_Backend/src/TutorUtils/Prompts/KnowledgePhase/Slides/slide_description_prompt",
    ...
    def from_sql(current_obj_idx, num_slides, slides, notebank, concept_database):
        ...
        slide_planer.Slides = [
            Slide.from_sql(
                ...
                s[5],
                s[6],
                [concept_database.get_concept(cpt) for cpt in s[7]],
            )
            for s in slides
        ]
        return slide_planer
    ...
    def generate_slide_plan(self):
        if DEBUG:
            print(f"Generating Slide Plan for {self.ConceptDatabase.main_concept}")
        notebank_state = self.Notebank.env_string()
        while True:
            # Prepare input for LLM
            slideplan_prompt = self.llm_api.prompt_plan_slideplan(
                self._generate_slideplans_str(),
                self._generate_concept_exploration_map_str(),
                notebank_state,
            )
            # Request output from LLM
            llm_output = self.llm_api.request_output_from_llm(
                slideplan_prompt, "gpt-4-1106-preview"
            )
            ...
            for i in range(5):
                try:
                    with open("translation.txt", "a") as f:
                        f.write("TRANSLATION\n")
                    llm_output = self.llm_api.request_output_from_llm(
                        conversion_prompt, model="gpt-3.5-turbo-16k"
                    )
                    slide_plan = SlidePlan.create_slideplan_from_JSON(
                        llm_output, self.ConceptDatabase
                    )
            ...
            slideplan_prompt = self.llm_api.prompt_terminate_slideplan(
                self._generate_slideplans_str(),
                self._generate_concept_exploration_map_str(),
                notebank_state,
            )
    ...
    def generate_slide(self, slide_plan: SlidePlan, index, results, generation_lock):
        ...
        slide_prompt = self.llm_api.prompt_sideplan_description(
            slide_plan, notebank_state
        )
        # Request output from LLM
        s_description = self.llm_api.request_output_from_llm(
            slide_prompt, "gpt-4-1106-preview"
        )
        ...
        system_prompt = DOC_SYSTEM_PROMPT.replace(
            "$ENV.SLIDE_PLAN$", slide_plan.env_string()
        ).replace("$ENV.NOTEBANK_STATE$", notebank_state)
        while True:
            with open("translation.txt", "a") as f:
                f.write("TRANSLATION\n")
            llm_output = self.llm_api.conversational_JSON_request(
                system_prompt, s_description, SLIDE_CONTENT_PROMPT, "gpt-4"
            )
        ...
        while True:
            with open("translation.txt", "a") as f:
                f.write("TRANSLATION\n")
            presentation_prompt = (
                SLIDE_PRESENTATION_PROMPT.replace("$TITLE$", slide_plan.title)
                .replace("$SLIDE_CONTENT$", s_content)
                .replace(
                    "$SLIDE_NUM$ of $TOTAL_SLIDES$", f"{index+1} of {self.num_slides}"
                )
            )
            llm_output = self.llm_api.conversational_JSON_request(
                system_prompt, s_description, presentation_prompt, "gpt-4-1106-preview"
            )
        ...
        n_slide = Slide(
            ...
            presentation=s_presentation,
            content=s_content,
            latex_codes="",
            purpose=slide_plan.purpose,
            purpose_statement=slide_plan.purpose_statement,
            concepts=slide_plan.concepts.copy(),
        )
        with generation_lock:  # Data sensitive
            results.append((index, n_slide))
            if DEBUG:
                print("Created Slide:", n_slide.format_json())
            self.num_slides += 1
    ...
    def _generate_slideplans_str(self):
        return "\n".join(
            [
                f"{slide_plan.title}: {slide_plan.purpose}, {slide_plan.purpose_statement}, Concepts: {', '.join([c.name for c in slide_plan.concepts if c is not None])}"
                for slide_plan in self.SlidePlans
            ]
        )
    ...
    def _generate_concept_exploration_map_str(self):
        return "\n".join(
            [
                f"{concept.name}: {sum([1 for slide_plan in self.SlidePlans if concept in slide_plan.concepts])}"
                for concept in self.ConceptDatabase.Concepts
            ]
        )
    ...
    def _generate_slide_window_context(self, idx):
        ...
        s += (
            """\n[Upcoming Slides]"""
            + f"""[{f'{newline+newline}'.join([f"Previous Slide: {newline}"+str(slide.format_json()) for i, slide in enumerate(self.SlidePlans[max(idx+1, self.num_slides-1):max(self.num_slides-1, idx+3)])])}]"""
            + "\n[/Upcoming Slides]"
